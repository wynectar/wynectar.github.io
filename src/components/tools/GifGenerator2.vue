<!-- 本地需要引入cdnjs.css，public需要放gif.worker.js -->
<template>
  <div class="gif-container">
    ...
    <main>
      ...
      <section>
        ...
        <div class="button-group">
          <button id="sampleBtn" class="btn btn-purple" @click="addSampleImages" :disabled="isGenerating">
            <i class="fas fa-magic"></i> 示例图片
          </button>
          <button id="clearBtn" class="btn btn-secondary" @click="clearAll" :disabled="images.length === 0">
            <i class="fas fa-trash-alt"></i> 清除图片
          </button>
        </div>

        <div class="upload-area" @click="triggerFileInput" @dragover.prevent="handleUploadDragOver"
          @dragleave="handleUploadDragLeave" @drop.prevent="handleUploadDrop" :style="uploadDragStyle">
          <i class="fas fa-cloud-upload-alt"></i>
          <div class="upload-text">点击选择或拖拽图片到这里</div>
          <div class="upload-subtext">支持 JPG、PNG、GIF 等格式，建议尺寸一致</div>
        </div>
        <input type="file" id="fileInput" class="file-input" multiple accept="image/*" @change="handleFileSelect"
          ref="fileInput" />

        <div class="image-list-container">
          <div class="image-list">
            <div v-for="(img, index) in images" :key="img.id" class="image-item" :class="{
              'dragging': draggingIndex === index,
              'drag-over': dragOverIndex === index,
              'drag-over-before': dragOverIndex === index && dragDirection === 'before',
              'drag-over-after': dragOverIndex === index && dragDirection === 'after'
            }" draggable="true" @dragstart="handleDragStart(index, $event)"
              @dragover.prevent="handleDragOver(index, $event)" @dragenter.prevent="handleDragEnter(index, $event)"
              @dragleave="handleDragLeave(index, $event)" @drop="handleDrop(index, $event)" @dragend="handleDragEnd"
              ...
              <button class="remove-btn" @click.stop="removeImage(index)"
                :style="{ opacity: hoveredIndex === index || draggingIndex === index ? 1 : 0 }">
                <i class="fas fa-times"></i>
              </button>
              <div class="frame-number">{{ index + 1 }}</div>
              <div class="drag-handle" title="拖拽排序" @mousedown="startDrag(index)">
      ...
      <section>
        ...
        <div class="button-group">
          <button id="previewBtn" class="btn btn-generate" @click="startPreview" :disabled="images.length < 2">
            <i class="fas fa-play"></i> 预览动画
          </button>
          <button id="generateBtn" class="btn" @click="generateGIF" :disabled="images.length < 2 || isGenerating">
        ...
        <div class="preview-container" :class="{ 'has-content': hasContent }">
          ...
          <canvas id="previewCanvas" ref="previewCanvas" :style="{ display: showCanvas ? 'block' : 'none' }"></canvas>
          <img id="gifPreview" ref="gifPreview" :src="generatedGifUrl" alt="生成的GIF"
        ...
        <div class="button-group">
          <button id="downloadBtn" class="btn btn-generate" @click="downloadGIF" :disabled="!generatedGifBlob">
            <i class="fas fa-download"></i> 下载GIF文件
          </button>
          <button id="downloadFramesBtn" class="btn" @click="downloadAllFrames" :disabled="imageElements.length === 0">
      ...
      <div class="controls">
        <div class="control-group">
          ...
          <div class="slider-container">
            <input type="range" v-model.number="delay" min="50" max="1000" step="50" @input="updateDelay" />
        ...
        <div class="control-group">
          <label>GIF尺寸：宽</label>
          <div class="slider-container">
            <input type="range" v-model.number="outputWidth" min="100" max="800" step="10" @input="updateSize" />
            <div class="range-info">
              <span>小</span>
        ...
        <div class="control-group">
          <label>GIF尺寸：高</label>
          <div class="slider-container">
            <input type="range" v-model.number="outputHeight" min="100" max="800" step="10" @input="updateSize" />
            <div class="range-info">
              <span>小</span>
    ...
    <div class="version">
      <i class="fas fa-code"></i> GIF生成工具 v2.0 | 基于HTML5 Canvas
    </div>
  </div>
</template>
...
// 计算属性
const uploadDragStyle = computed(() => ({
  borderColor: uploadIsDragging.value ? "#2980b9" : "#3498db",
  background: uploadIsDragging.value ? "#f0f9ff" : "#fff",
  transform: uploadIsDragging.value ? "translateY(-3px)" : "translateY(0)"
}));
...
const handleDragOver = (index, event) => {
  ...
  dragDirection.value = relativeY < middle ? 'before' : 'after';
  dragOverIndex.value = index;
};
...
const handleDrop = (targetIndex, event) => {
  ...
  if (draggingIndex.value === -1 || draggingIndex.value === targetIndex) return;
...
const reorderImages = (fromIndex, toIndex) => {
  ...
  if (dragDirection.value === 'before' && toIndex > fromIndex) {
    insertIndex = toIndex - 1;
  } else if (dragDirection.value === 'after' && toIndex < fromIndex) {
    insertIndex = toIndex + 1;
  }

  // 确保索引有效
  insertIndex = Math.max(0, Math.min(insertIndex, newImages.length));
...
.image-item.dragging .drag-handle {
  opacity: 1;
}
...
.image-item.dragging .remove-btn {
  opacity: 1;
}
